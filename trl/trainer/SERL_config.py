...
@dataclass
class SERLConfig(TrainingArguments):
    r"""
    ...
    Parameters:
        ...
        N_num_generations_actor (`int` or `None`, *optional*, defaults to `8`):
    ...
    max_prompt_length_p2: Optional[int] = field(
        default=512,
        metadata={
            "help": "Maximum length of the prompt. If the prompt is longer than this value, it will be truncated left."
        },
    )
    N_num_generations_actor: Optional[int] = field(
        default=8,
        metadata={
    ...
    K_num_generations_judge: Optional[int] = field(
        default=8,
        metadata={
            "help": "Number of generations to sample. The effective batch size (num_processes * per_device_batch_size "
            "* gradient_accumulation_steps) must be evenly divisible by this value."
    ...
    max_completion_length_p2: Optional[int] = field(
        default=256,
        metadata={"help": "Maximum length of the generated completion."},
    )
    ...
    def __post_init__(self):
        ...
        if self.N_num_generations_actor < 2 or self.K_num_generations_judge < 2:
            raise ValueError(
                "SERL requires at least 2 generations per prompt to calculate the advantages. You provided "
                f"{self.N_num_generations_actor}, which is less than the minimum required."
                f"{self.K_num_generations_judge}, which is less than the minimum required."
            )
        possible_values = [
            n_gen for n_gen in range(2, self.generation_batch_size + 1) if (self.generation_batch_size) % n_gen == 0
        ]

        if self.N_num_generations_actor not in possible_values or self.K_num_generations_judge not in possible_values:
            raise ValueError(
                ...
                f"prompt ({self.N_num_generations_actor}). Given the current effective train batch size, the valid values for "
                f"prompt ({self.K_num_generations_judge}). Given the current effective train batch size, the valid values for "
        ...
        if self.eval_strategy != "no":
            ...
            if self.N_num_generations_actor not in possible_values or self.K_num_generations_judge not in possible_values:
                raise ValueError(
                    ...
                    f"evenly divisible by the number of generations per prompt ({self.N_num_generations_actor}). Given the "
