...
def extract_summary(completion):
    marker = '</think>\n\n'
    if marker in completion:
        return completion.split(marker, 1)[1]
    else:
        return completion
    
def extract_boxes(completions):
    results = []
    num_none = 0
    num_wrongbox = 0
    for completion in completions:
        ...
        if match:
            if(match.group(1) != "1" and match.group(1) != "2"):
                num_wrongbox += 1
    ...
    none = num_none/length
    wrongbox = num_wrongbox/length
    return results, none, wrongbox

def get_prompt_str(context: str) -> str:
    prompt = (
        f"Here's a writing prompt:\n"
        f"{context}\n\n"
        "Write a story continuation of about 200 words. "
        "Develop the narrative by introducing characters, emotions, or conflicts. "
        "Keep the tone consistent with the original prompt and maintain an engaging storytelling style.\n\n"
        "Story continuation:"
    )
    return prompt

def get_prompt_classification_Criteria(context: str, context_1: str, context_2: str) -> str:
    instruction = json.dumps({"instruction": context}, ensure_ascii=False, indent=4)
    output_1 = json.dumps({"output": context_1}, ensure_ascii=False, indent=4)
    output_2 = json.dumps({"output": context_2}, ensure_ascii=False, indent=4)

    prompt = (
        "I require a leaderboard for various large language models. I'll provide you with prompts given to these "
        "models and their corresponding outputs. Your task is to assess these responses and select the model that "
        "produces the best output from a human perspective.\n"
        "## Instruction\n"
        "{instruction}\n"
        "\n"
        "## Model Outputs\n"
        "Here are the unordered outputs from the models. Each output is associated with a unique model identifier.\n"
        "{{\n"
        "    {{\n"
        '        "model.identifier": "1",\n'
        "        {output_1}\n"
        ...
        "\n"
        "## Task\n"
        "Evaluate the models based on the quality and relevance of their outputs, and select the model that generated "
        "the best output. Answer by providing the model identifier of the best model. We will use your output as the "
        "name of the best model, so make sure your output only contains one of the following model identifiers and nothing else.\n"
        "model 1 is better: output as \\boxed{{1}}\n"
        "model 2 is better: output as \\boxed{{2}}\n"
        "Output the best model name within \\boxed{{}}\n"
    ).format(
